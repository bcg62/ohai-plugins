    ...
    network['interfaces'].each do |iface, _iface_v|
      ...
      lldptool.each_pair do |key, value|
        ...
        case key
          when 'sysName', 'MTU'
            so.stdout.split("\n").each do |line|
              lldp[iface][key] = $1 if line.match(/^\s+(.*)/)
            end
          when 'chassisID'
            so.stdout.split("\n").each do |line|
              lldp[iface][key] = $1 if line.match(/MAC:\s+(.*)/)
            end
          when 'portID'
            so.stdout.split("\n").each do |line|
             lldp[iface][key] = $1 if line.match(/(?:Ifname|Local):\s+(.*)/)
            end
          when 'mngAddr_ipv4'
            so.stdout.split("\n").each do |line|
              lldp[iface][key] = $1 if line.match(/IPv4:\s+(.*)/)
            end
          when 'mngAddr_ipv6'
            so.stdout.split("\n").each do |line|
              lldp[iface][key] = $1 if line.match(/IPv6:\s+(.*)/)
            end
          when 'PVID'
            so.stdout.split("\n").each do |line|
             lldp[iface][key] = $1 if line.match(/(?:Info|PVID):\s+(.*)/)
          end
        end
      end
    end
